    ...
    <header className="sticky top-0 z-50 w-full bg-white border-b border-gray-200">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          ...
          <Link href="/joinus" className="bg-black text-white hover:bg-gray-800 px-4 py-2 rounded-2xl">
            Join Now
          </Link>
        </nav>
      </div>
      ...
      {isOpen && (
        <nav className="md:hidden bg-white border-t border-gray-200">
          <ul className="flex flex-col space-y-4 py-4 px-4">
            ...
            <li>
              <Link href="/#ecosystem" className="text-sm font-medium text-gray-600 hover:text-black" onClick={() => setIsOpen(false)}>
                Our Ecosystem
              </Link>
            </li>
            <li>
              <Link href="/joinus" className="bg-black text-white hover:bg-gray-800 px-4 py-2 rounded-2xl block text-center" onClick={() => setIsOpen(false)}>
                Join Now
              </Link>
            </li>
          </ul>
